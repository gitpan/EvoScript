...
=head1 Text::PropertyList

Text::Propertylist provides functions that turn data structures of nested references into text and back again.  Uses NeXT's PropertyList format (see Caveats).  Useful for saving and loading data in flat files, and generating error messages in debugging.
...
We've added an alternate multiline syntax, <<DELIMITER.

The fromtext function assumes that the top-level is a hash.
